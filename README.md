# 360-ATX-Power-Board
ATX to Xbox power adapter board for Xbox 360
...
# Compatibility
Any Xbox 360, provided you have some way of getting power from the screw terminals into the console itself. I tested this with a Xenon. The most power-hungry model (Xenon) came with a 203W power adapter, so a 300W or higher ATX PSU should power without running into current limits.

This theoretically could work with an Xbox One but I don't have one to test.

# Prerequisites
- An ATX power supply capable of supplying at least 220W over 12V
- The power cable from an Xbox 360 power brick or compatible replacement
- Fuses (automotive style ATO fuses), 2A and 20A

# Instructions
1. Desolder or otherwise harvest the device-side power cable (i.e. not the wall end) from an Xbox 360 power brick or compatible; I think you can get the connector itself on Aliexpress and fabricate your own cable with thick enough wiring to handle the current
...
7. Set switch SW1 (if installed) to on
8. Turn on Xbox 360; all LEDs should light up, as should your Xbox

# Building the PCB
Import project files into KiCad 10 and export to your PCB manufacturing service of choice, or just use the files in the grbl subfolder. It's a four layer PCB and it'll carry quite a bit of current, so I recommend thicker than default copper.
...
- Components
  - Power signaling
    ...
    - R7, R8: 2.2K, 1206
    - U1: Texas Instruments SN74LVC1G14DBVR or compatible part
    - C1: 1uF 1206 25V
    - C2: 0.1uF 1206 25V
  - Switching
  ...
  - Filtering:
    - C3-C4: 100uF 25V electrolytics, 6.3mm x 7.7mm footprint. Würth Elektronik 865080445010 or compatible.
  - Indicators
    - Optional blinkenlights to see if each power rail is working. Install the following resistor-LED pairs:
    - +12V
      - R3: 1K, 1206
      - D3: 20mW LED, 1206  
    - +5V
    ...
    - -12V
      - R6: 1K, 1206
      - D6: 20mW LED, 1206 (note the reversed orientation compared to the others!)
    - PWR_OK
      - R2: 330R, 1206
      - D2: 20mW LED, 1206
  - 5V Load
    - Since the 360 is almost exclusively 12V load, some PSUs may shut down prematurely due to lack of 5V load. If you have such a PSU or just want to play it safe, install the following to have a 5W (1A) load:
